...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        def find_clear_annotation_area(ax, x_data_sets, y_data_sets, avoid_boxes=None, annotation_type="detection"):
            ...
            # Analyze data distribution
            density_map, clear_regions = analyze_data_distribution(x_data_sets, y_data_sets, (x_min, x_max),
                                                                   (y_min, y_max))
        ...
        def plot_contrast_curve_full(results, title="Contrast Curve", telescope_name="Hooker",
            ...
            info_text.append(f'Wavelength: {wavelength * 1e9:.0f} nm')
            info_text.append(
                f'Pixel scale: {results["pixel_scale"]:.4f}"/pix ({results["pixel_scale"] * 1000:.1f} mas/pix)')
            info_text.append(f'PSF FWHM: {results["fwhm_arcsec"]:.3f}" ({results["fwhm_pixels"]:.1f} pix)')
            info_text.append(
        ...
        class FinalContrastCurveGUI:
            ...
            def create_widgets(self):
                ...
                # Target Name
                ttk.Label(param_frame, text="Target Name:").grid(row=0, column=0, sticky=tk.W)
                ttk.Entry(param_frame, textvariable=self.target_name, width=30).grid(row=0, column=1, columnspan=3,
                                                                                     sticky=tk.W)
                ...
                filter_frame.grid(row=6, column=0, columnspan=4, pady=5)
                ttk.Label(filter_frame, text="Filter Presets:").pack(side=tk.LEFT, padx=5)
                ttk.Button(filter_frame, text="Sloan r'", command=lambda: self.set_filter(617)).pack(side=tk.LEFT,
                                                                                                     padx=2)
                ttk.Button(filter_frame, text="Sloan i'", command=lambda: self.set_filter(748)).pack(side=tk.LEFT,
                                                                                                     padx=2)
