...
def find_clear_annotation_area(ax, x_data_sets, y_data_sets, avoid_boxes=None, annotation_type="detection"):
    ...
    # Define margins once for all iterations
    margin_x = 0.08 * (x_max - x_min)
    margin_y = 0.08 * (y_max - y_min)
    ...
    for rel_x, rel_y, pos_name in candidate_positions:
        ...
        # 5. Ensure box stays within plot bounds (now redundant but kept for extra safety)
        if (box_x_min < x_min + margin_x or box_x_max > x_max - margin_x or
            box_y_min < y_min + margin_y or box_y_max > y_max - margin_y):
            score -= 5  # Moderate penalty since we now enforce bounds above
    ...
    # If no good position found, use fallback that definitely stays within bounds
    if best_position is None or best_score < 0:
        # Conservative fallback position with guaranteed bounds compliance
        margin_x = 0.08 * (x_max - x_min)
        margin_y = 0.08 * (y_max - y_min)
        box_width = box_width_rel * (x_max - x_min)
        box_height = box_height_rel * (y_max - y_min)

        # Place in upper right with sufficient margin
        fallback_x = x_max - box_width - margin_x
        fallback_y = y_max - margin_y

        # Double-check bounds and adjust if needed
        if fallback_x < x_min + margin_x:
            fallback_x = x_min + margin_x
        if fallback_y - box_height < y_min + margin_y:
            fallback_y = y_min + box_height + margin_y
...
def plot_contrast_curve_full(results, title="Contrast Curve", telescope_name="Hooker",
    ...
    if len(max_seps) > 0:
        ...
        if np.any(detections):
            ...
            # Instead of circling detections, just add them to legend without special marker
            # They'll be visible as regular maxima points that happen to be below the limit
            print(f"\nFound {np.sum(detections)} detections below {confidence_level}σ limit")

            # Annotate the brightest detection with smart positioning (arrow only)
            if len(det_mags) > 0:
                ...
                # Create annotation with arrow pointing to detection
...
class FinalContrastCurveGUI:
    ...
    def __init__(self, root):
        ...
        self.root.geometry("800x700")  # Reduced height since save section is removed
    ...
    def create_widgets(self):
        ...
        ttk.Button(filter_frame, text="Sloan g'", command=lambda: self.set_filter(475)).pack(side=tk.LEFT, padx=2)
        ...
        ttk.Button(filter_frame, text="Sloan z'", command=lambda: self.set_filter(893)).pack(side=tk.LEFT, padx=2)
        ...
        main_frame.rowconfigure(3, weight=1)  # Changed from row 4 to row 3
    ...
    def set_filter(self, wavelength_nm):
        ...
        filter_names = {475: "Sloan g'", 617: "Sloan r'", 748: "Sloan i'", 893: "Sloan z'"}
        filter_name = filter_names.get(wavelength_nm, f"Filter {wavelength_nm}")
